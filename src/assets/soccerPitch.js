...
function soccerPitch (props) {
    ...
    this.postJson=function() {
        ...
        if (confirm('Enregistrement compo en cours')){
            $.ajax({
                ...
                alert('Impossible de sauver la compo:'+textStatus+'-'+errorThrown);
            });
        }
    }
    ...
    this.loadData=function(mode){
        ...
        $.getJSON('/jsonsoccer?user='+userN+'&mode='+mode+'&sheetName='+encodeURIComponent(obj.sheetName), function (result) {
             if (result) {
              var tmp;
                ...
                $.each(result, function(i, item){
                    if (item.sheetName)
                        if (item.sheetName.length>0)
                            if(item.sheetName==obj.sheetName && item.user==obj.user && item.user==obj.userView){
                                links+="<li><a class='_sp_class_current' href='/soccer?sheetName="+encodeURIComponent(item.sheetName)+"'>"+item.sheetName+"</a> &nbsp;&nbsp;<a class='_sp_class_current' href='/delete?obj="+item._id+"' >Suppr. </a></li>";
                            }
                            else {links+="<li><a class='_sp_class_link' href='/soccer?sheetName="+encodeURIComponent(item.sheetName)+"'>"+item.sheetName+"</a> &nbsp;&nbsp;<a class='_sp_class_link' href='/delete?obj="+item._id+"' >Suppr. </a></li>";}
                });
                document.getElementById('result').innerHTML="<span class='_sp_class_link'>Mes compos enregistrees:<br><ol>"+links+"</ol></span>";
                $.getJSON('/getOtherUsers',function(result2){
                    if (result2){
                        $.each(result2, function(ind, el){
                            if (el.username)
                                if (el.username.length>0)
                                    if(el.username!=obj.user){
                                        linksUser+="<li><a class='_sp_class_link' href='/getSheetsFromUser?user="+el.username+"' >"+el.username+"</a></li>";
                                    }
                        });
                        document.getElementById('result').innerHTML+="<br><span class='_sp_class_link'>Les autres utilisateurs enregistres:<br><ol>"+linksUser+"</ol></span>";
                    }
                });
            }
        });
    ...
    this.displaySheetsFromUser = function (user,data){
        var links='';
        var obj=this;
        if (data[0]==''){
            data[0]=obj.sheetName; 
        }
        for (var i=0;i<data.length;i++){
            links+="<li><a class='_sp_class_link' href='/soccer?user="+obj.user+"&sheetName="+encodeURIComponent(data[i])+"&mode=viewOther&userview="+obj.userView+"'>"+data[i]+"</a></li>";
        }
        document.getElementById('result2').innerHTML="<br><span class='_sp_class_link'>Voir Les compos de "+user+"<br><ol>"+links+"</ol></span>";
        /*
        $.getJSON('/getSheetsFromUser?user='+user,function(result){
            if (result){
                $.each(result, function(i, item){
                    if (item.sheetName)
                        if (item.sheetName.length>0)
                            links+="<li><a class='_sp_class_link' href='#' onclick='loadSheetOther(user,item.sheetName+)'>"+item.sheetName+"</a></li>";
                });
                document.getElementById('result').innerHTML+="<br><span class='_sp_class_link'>Les feuilles de <br><ol>"+links+"</ol></span>";
            }
        });*/
    }   
    ...
    if (this.configMenu!==false) {
        this.htmlBottomMenu+="<span class='_sp_class_obj'>Enregistrer la compo sous:</span><input id='sheetName' type='text' value='"+this.lastProperties.sheetName+"' placeholder='sheetName'> ";
