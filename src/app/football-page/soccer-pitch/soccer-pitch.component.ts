...
export class SoccerPitchComponent implements OnInit {
    ...
    ngOnInit() {
        this.sub=this.route.params
            .subscribe((val)=>{
                this.game_id = parseInt(val.id);
                this.awayTeamId=parseInt(val.awayTeamId);
                this.homeTeamId=parseInt(val.homeTeamId);
    ...
    onChanges(){
        this.reactiveMenuForm.get('team1Direction').valueChanges.subscribe(val=>{
            if (!this.bUpd1){
                this.bUpd2=true;
                this.reactiveMenuForm.get('team2Direction').patchValue((parseInt(val)%2===0)?'1':'0');
                this.bUpd2=false;
            }
        });
        this.reactiveMenuForm.get('team2Direction').valueChanges.subscribe(val=>{
            if (!this.bUpd2){
                this.bUpd1=true;
                this.reactiveMenuForm.get('team1Direction').patchValue((parseInt(val)%2===0)?'1':'0');
                this.bUpd1=false;
            }
        });
    }
    ...
    saveProperties(){
        ...
        if (!props._id){
            ...
            this.updateObs=this.soccerPitchDb.updateProperties(props)
                .subscribe((resp)=>{
                    console.log(resp);
                    if (resp.ok) this.notif.notify('Composition Modifiée avec succès','SUCCESS',2000); 
    ...
    deleteProperties(){
        ...
        if (this.userSrv.getConnectedUser()===this.properties.user){
            ...
            this.delObs.subscribe(resp=>{
                this.delResp=resp;
                if (this.delResp.n==1){
                    this.propsResp.splice(this.propsResp.findIndex(val=>{return val._id==this.properties._id}),1);
                    this.previous();
    ...
    updateAll(){
        ...
        this.cnv.nativeElement.setAttribute('title',this.properties.user+' - '+this.properties.sheetName +' - '+this.properties._id);
        this.updatePicture();
    }
    previous(){
        if (this.ind===0){
...
//Raffraichissement Image
    updatePicture = function(){
        ...
        let cntDn;
        this.draw();
        let canv=this.cnv.nativeElement;
        if (this.properties.teams[up])
    ...
    writePlayer = function (player){
        ...
        else context.fillText( player.name, player.x,( player.y + (this.properties.playerSize*2)));
    }
    onValidateConfig(){
        this.properties.type=this.reactiveMenuForm.value.type;
        this.properties.playerSize=parseInt(this.reactiveMenuForm.value.playerSize);
    ...
    //   }
        ...
        this.properties.teams[0].stripesType=parseInt(this.reactiveMenuForm.value.stripesTypeTeam1);
        this.properties.teams[0].textColor=this.reactiveMenuForm.value.team1TextColor;
        this.properties.teams[0].direction=parseInt(this.reactiveMenuForm.value.team1Direction);
        this.properties.teams[0].numberOfStripes=parseInt(this.reactiveMenuForm.value.numberOfStripes1);
        ...
        this.properties.teams[1].stripesType=parseInt(this.reactiveMenuForm.value.stripesTypeTeam2);
        this.properties.teams[1].textColor=this.reactiveMenuForm.value.team2TextColor;
        this.properties.teams[1].direction=parseInt(this.reactiveMenuForm.value.team2Direction);
        this.properties.teams[1].numberOfStripes=parseInt(this.reactiveMenuForm.value.numberOfStripes2);
    ...
    onValidatePlayer(){
        ...
        let indPl=this.findSelClkPlayer(this.reactivePlayerForm.value.x,this.reactivePlayerForm.value.y);
        //console.log(indPl);
        //console.log(this.properties.teams);
        this.properties.teams[this.getDirection(this.reactivePlayerForm.value.y)].players[indPl].name=this.reactivePlayerForm.value.playerName;
        this.properties.teams[this.getDirection(this.reactivePlayerForm.value.y)].players[indPl].goals=this.reactivePlayerForm.value.goals;
        this.properties.teams[this.getDirection(this.reactivePlayerForm.value.y)].players[indPl].ycards=this.reactivePlayerForm.value.yCards;
        this.properties.teams[this.getDirection(this.reactivePlayerForm.value.y)].players[indPl].rcards=this.reactivePlayerForm.value.rCards;
        this.updatePicture();
    ...
    draw() {
        ...
        let bandWth = this.height / this.properties.grassStripesNumber;
        ...
        let xcenter = this.properties.top.x + (this.properties.width / 2);

        if (this.properties.type==halfPitch){
            yend = this.properties.top.y + Math.round(this.height*cutPt);
            if (this.properties.direction==up) yend = Math.round(this.height*cutPt);
        } 
        else {
            yend = this.properties.top.y + this.height;
        }
        let xend = this.properties.top.x + this.properties.width;
        ...
        else ycenter = Math.round(this.properties.top.y + (this.height / 2));


       // console.log(ycenter);
        let scaleMetersPx = this.properties.width / this.properties.widthIRL;
        ...
        let centralCircleRad=(9.15* (this.properties.width / this.properties.widthIRL));
        let context: CanvasRenderingContext2D = canvas.getContext("2d");
        ...
        if (this.properties.type==halfPitch ) {
            context.fillRect(0, 0, this.properties.width+(this.properties.top.x*2), (this.height*2/3)+(this.properties.top.y*2) );
        } else {
            context.fillRect(0, 0, this.properties.width+(this.properties.top.x*2), (this.height)+(this.properties.top.y*2) );
        }
        let tmpY;
        if (this.properties.type==halfPitch && this.properties.direction==up)
    ...
    drawPlayer(player,dir){
        let cnv=this.cnv.nativeElement;
        let imgGl=new Image();
        let imgYc=new Image();
        let imgRc=new Image();
        ...
        let nbStr=this.properties.teams[dir].numberOfStripes;
        let symetricAxisAngle, angle, symetricAngle;
        let R=this.properties.playerSize;  
        ...
        let shiftDist=this.properties.playerSize;
        let obj=this;
        imgGl.onload = function(){
            ...
            for (let i=0; i < player.goals; i++){
                ...
                else context.drawImage(imgGl,0,0,12,12, startX+12*i, player.y-shiftDist*2.3  ,12,12 );
            }
        };
        imgYc.onload = function(){
            
            for (let i=0; i < player.ycards; i++){
                let X1=player.x-12 - 2*shiftDist*(i+1);
                let X2=player.x + 2*shiftDist*(i+1);
        ...
        imgRc.onload = function(){
            for (let i=0; i < player.rcards; i++){
                let X1=player.x + 2*shiftDist*(i+1);
                let X2=player.x-9 - 2*shiftDist*(i+1);
    ...
    addPlayer(X,Y,dir){
        ...
        let scale=this.properties.widthIRL/this.properties.width;
        if (this.properties.type !=halfPitch){
    ...
    findSelClkPlayer = function (x,y){
        ...
        if (this.properties.teams){
            if (this.properties.teams.length>0){
                if (this.properties.teams[up]){
                    for (var i=0; i < this.properties.teams[up].players.length; i++ ){
                        d=(x-this.properties.teams[up].players[i].x) * (x-this.properties.teams[up].players[i].x) + (y-this.properties.teams[up].players[i].y)*(y-this.properties.teams[up].players[i].y);
                ...
                if (this.properties.teams[down]){
                    for (var i=0; i < this.properties.teams[down].players.length; i++ ){
                        d=(x-this.properties.teams[down].players[i].x) * (x-this.properties.teams[down].players[i].x) + (y-this.properties.teams[down].players[i].y)*(y-this.properties.teams[down].players[i].y);
                        if ( d < this.sqrDistBtwPl) {
                            return i;
                        }
                    }
    ...
    removePlayer = function(x,y){
        let indice=this.findSelClkPlayer(x,y);
        if (indice > -1) {
    ...
    calculateOffsets(){
        ...
        let offsetLeft=this.cnv.nativeElement.offsetLeft;
        let offsetTop=this.cnv.nativeElement.offsetTop;
    ...
    addPlayerEvt(e){
        let shift=obj.calculateOffsets();
        let cX=e.pageX-shift.offsetLeft;
        let cY=e.pageY-shift.offsetTop;
        ...
        if(!obj.editingMode){
            ...
            let indPl=obj.findSelClkPlayer(cX,cY);
            if(indPl>-1){
