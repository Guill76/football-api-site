...
export class LiveService {
  ...
  init(){
    ...
    this.resObs=this.fdbServ.getObsRequest('fixtures').subscribe(res=>{
      this.show=true;
      this.liveResults=res.fixtures.filter((val)=>{return val.status==='IN_PLAY'});
      this.liveComing=res.fixtures.filter((val)=>{return (val.status==='TIMED'&& val.date > now.toISOString())});
